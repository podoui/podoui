...
export const useYoutubeEditor = ({
   ...
   * 유튜브 리사이즈 시작 (레거시 방식)
   */
  const startYoutubeResize = useCallback(
    (e: MouseEvent, container: HTMLElement, handle: string) => {
  ...
  const deleteYoutube = useCallback(() => {
    ...
    while (parent && parent !== editorRef.current) {
      if (
        parent.classList.contains("youtube-wrapper") ||
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      switch (resizeStartData.handle) {
        ...
        case "w":
          newWidth =
            resizeStartData.startWidth + (resizeStartData.handle === "e" ? deltaX : -deltaX);
          newHeight = newWidth / aspectRatio;
          break;
        case "n":
        case "s":
          newHeight =
            resizeStartData.startHeight + (resizeStartData.handle === "s" ? deltaY : -deltaY);
