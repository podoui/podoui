...
const USAGE_TABS: CodeTab[] = [
  ...
  },
  {
    target: "native",
    label: "React Native",
    code: nativeComponentUsage(["Button"], `<Button theme="solid-primary">Save</Button>`),
  },
];
  ...
  return (
    <>
      ...
      <DocSection
        index={5}
        title="속성 (props)"
        description="podo-ui/react의 Button이 받는 속성이에요. 앞서 시안의 layout(text·prefix·suffix)은 코드에서는 prefix·suffix 두 슬롯(ReactNode)으로 표현하며, 둘을 동시에 넣을 수도 있어요. 이 밖에 표준 button 속성(className, aria-*, type 등)도 그대로 전달돼요."
      >
        <SpecTable
          variant="props"
