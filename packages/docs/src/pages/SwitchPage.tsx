  ...
  return (
    <>
      ...
      <DocSection
        index={4}
        title="속성 (props)"
        description="podo-ui/react의 Switch가 받는 속성이에요. 시안의 state(on·off)는 코드에서 웹 표준(role=switch, aria-checked)에 맞춘 checked로 표현돼요. checked를 생략하면 스스로 상태를 관리하는 비제어형으로 동작해요. 이 밖에 표준 button 속성(className, aria-* 등)도 그대로 전달돼요."
      >
        <SpecTable
          variant="props"
