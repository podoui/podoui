...
- [x] 문서 코드 예제 프레임워크 전환 개선
  - 완료 기준: 모든 프레임워크 탭에서 줄 번호 열이 안정적으로 정렬되고 긴 코드의 가로 스크롤 중에도 번호가 유지된다.
...
- [x] Hono 기반 문서 clean URL과 페이지 전환 스크롤 복원
  - 완료 기준: 문서 내부 링크와 직접 접속 주소가 `#/…` 대신 `/…` 경로를 사용하며 기존 해시 주소는 clean URL로 자동 교체된다.
  - 완료 기준: 내부 페이지 전환과 브라우저 앞/뒤 이동 시 새 문서의 최상단으로 이동한다.
  - 완료 기준: Cloudflare Worker의 Hono 진입점이 정적 자산을 제공하고 확장자 없는 문서 경로를 SPA 셸로 폴백해 직접 접속·새로고침을 지원한다.
  - 완료 기준: React, Next.js, Hono CSR/SSR, React Native별 프로젝트 전체 테마 적용과 의미 기반 색상 토큰 사용 예제를 제공하고 필요한 공개 API를 검증한다.
  - 완료 기준: 라우팅·Worker 테스트, 전체 품질 검사, 브라우저 검증 및 Codex strict review가 통과한다.
...
- [ ] Code Connect publish
  ...
  - (2026-07-22 상태: publish 직전까지 완료 — import 해석·원본 파일 키 교체(uaLVvCUnvoWj4oz6ZMXxwP)·Field footer 매핑까지 끝나 `figma connect publish` 검증 "All Code Connect files are valid". 업로드만 403: PODO 계정에서 전 스코프 체크로 재발급해도 **PAT 화면에 Code Connect 스코프 항목 자체가 미노출** — Code Connect는 Organization/Enterprise 플랜 + Full/Dev 시트 전용이라, 디자인 파일이 속한 조직의 플랜 또는 PODO 계정 시트가 미달인 상태로 확인됨. 남은 것: 플랜/시트 확인 후 ① Org 플랜+Full/Dev 시트 확보 → 재발급 토큰으로 publish 1회, 또는 ② 플랜 업그레이드 전까지 보류(parse 정합성은 `pnpm figma:check`로 CI 검증 지속).)
